// Command smoke is the end-to-end smoke test for the go-Calc agent control
...
//
//	go run ./tools/smoke
//	go run ./tools/smoke -base-url https://127.0.0.1:8737 -api-key <key>
...
// call performs a request and returns (status, decoded JSON). A body of any
// non-nil type is sent as JSON. key overrides the client's API key when
// non-empty (used to test auth rejection).
...
func display(state any) string {
	d, _ := asMap(state)["display"].(string)
	return d
}
...
func main() {
	baseURL := flag.String("base-url", "", "e.g. https://127.0.0.1:8737 (default: derived from settings.json)")
	...
	ch.ok("ax advertises capabilities", hasCalc, "")
	...
	// --- direct compute ---
	calc := func(expr string) (int, any) {
		st, body, _ := c.call("POST", "/v1/calc", map[string]string{"expression": expr}, "")
		return st, body
	}
	result := func(body any) string {
		r, _ := asMap(body)["result"].(string)
		return r
	}

	st, body = calc("2 + 3 * 4")
	ch.ok("calc respects precedence (2 + 3 * 4 = 14)", st == 200 && result(body) == "14", fmt.Sprint(body))

	st, body = calc("9007199254740992 + 1")
	ch.ok("calc exact beyond 2^53", st == 200 && result(body) == "9007199254740993", fmt.Sprint(body))

	st, body = calc("0.1 + 0.2")
	ch.ok("calc exact decimals (0.1 + 0.2 = 0.3)", st == 200 && result(body) == "0.3", fmt.Sprint(body))

	st, body, _ = c.call("POST", "/v1/calc", map[string]string{}, "")
	ch.ok("calc empty body -> 400 missing_field",
		st == 400 && errCode(body) == "missing_field", fmt.Sprintf("status=%d body=%v", st, body))

	st, body = calc("1 / 0")
	ch.ok("calc div-by-zero -> 422 calculation_error",
		st == 422 && errCode(body) == "calculation_error", fmt.Sprintf("status=%d body=%v", st, body))
	...
	ch.ok("ui state reachable on calc view", st == 200 && view == "calc", fmt.Sprintf("status=%d", st))

	c.press("key-C")
	c.press("key-7")
	_, state, _ = c.press("key-9")
	ch.ok("pressing real buttons appends (7,9 -> '79')", display(state) == "79",
		fmt.Sprintf("display=%q", display(state)))

	c.press("key-C")
	for _, k := range []string{"7", "*", "6"} {
		c.key_(k)
	}
	_, state, _ = c.key_("Enter")
	ch.ok("keyboard drive + evaluate (7 * 6 = 42)", display(state) == "42",
		fmt.Sprintf("display=%q", display(state)))
	...
	_, state, _ = c.key_("Enter")
	ch.ok("parentheses via keys ((2 + 3) * 4 = 20)", display(state) == "20",
		fmt.Sprintf("display=%q", display(state)))
	c.press("key-C")
	...
	// an unstructured response is not. Point GO_CALC_UPDATE_URL at a stub when
	// launching the app to run this without touching the real GitHub.
	...
	// describe dynamic patterns (remove-<cidr>), and conditional controls only
	// exist in specific states (an error being shown, HTTPS enabled).
	conditional := map[string]bool{
		"server-error": true, "ip-error": true,
		"fingerprint": true, "copy-fingerprint": true,
		"curl-example": true, "copy-curl": true,
		// update result card: rendered only while an update is available
		"update-notes": true, "update-install": true,
		"update-skip": true, "update-later": true,
	}
	...
	_, state, _ = c.press("back") // calc view
	for t := range controls(state) {
		seen[t] = true
	}
	var missing []string
