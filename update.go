...
// GET /v1/update always agree. Ported from go-notepad (docs/updater-design.md
// over there).
...
const (
	updateRepo = "viniciusbuscacio/go-calc"
...
// GO_CALC_UPDATE_URL is set (tests / smoke runs).
func (a *App) updateClient() *updater.Client {
	return &updater.Client{
		BaseURL: os.Getenv("GO_CALC_UPDATE_URL"),
		Repo:    updateRepo,
		App:     "go-calc",
	}
}

// mutateSettings applies fn under the lock, then persists the result outside
// it (same locking discipline as the setters in app.go).
func (a *App) mutateSettings(fn func(*settings.Settings)) error {
	a.mu.Lock()
	fn(&a.cfg)
	cfg := a.cfg
	a.mu.Unlock()
...
func (a *App) CheckForUpdates() UpdateInfo {
	...
	_ = a.mutateSettings(func(s *settings.Settings) { s.UpdateLastCheck = now })
...
func (a *App) InstallUpdate() error {
	...
	// calculator has no unsaved state to flush.
...
func (a *App) SkipUpdateVersion() error {
	...
	if err := a.mutateSettings(func(s *settings.Settings) { s.UpdateSkipped = "v" + info.Version }); err != nil {
		return err
	}
...
func (a *App) RemindUpdateLater() error {
	...
	if err := a.mutateSettings(func(s *settings.Settings) { s.UpdateLaterUntil = until }); err != nil {
		return err
	}
	a.setUpdState(func(u *UpdateInfo) {})
	return nil
...
func (a *App) SetUpdateAutoCheck(v bool) error {
	return a.mutateSettings(func(s *settings.Settings) { s.UpdateAutoCheck = v })
}
