...
func TestAssetName(t *testing.T) {
	c := &Client{App: "go-calc"}
	name := c.assetName("v0.2.0")
	if !strings.HasPrefix(name, "go-calc-v0.2.0-") {
		t.Fatalf("assetName = %q, want go-calc-v0.2.0-<os>-<arch>", name)
	}
...
func fakeRelease(t *testing.T, tag string, binContent []byte, tamper bool) *httptest.Server {
	...
	if strings.HasSuffix(assetName, ".tar.gz") {
		archive = makeTarGz(t, "go-calc/"+binName, binContent)
	} else {
	...
	mux.HandleFunc("/repos/user/go-calc/releases/latest", func(w http.ResponseWriter, r *http.Request) {
